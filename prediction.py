...
def vids(
     vae_path, ed_path, root_dir="sample_prediction_data",dataset=None, num_frames=15, net=None, fp16=False
):
    result = set_result() 
    r = 0
    f = 0
    count = 0
    model = load_genconvit(net, fp16,vae=vae_path, ed=ed_path)
...
def gen_parser():
    ...
    return path, dataset, num_frames, net, fp16, vae_name, ed_name


def main():
    start_time = perf_counter()
    path, dataset, num_frames, net, fp16, vae_name, ed_name = gen_parser()
    result = (
        globals()[dataset](path, dataset, num_frames, net, fp16)
        if dataset in ["dfdc", "faceforensics", "timit", "celeb"]
        else vids(vae_name, ed_name, path, dataset, num_frames, net, fp16)
    )
